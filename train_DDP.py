...
def save_loss_plot(total_loss_info_list,cfg, logger):
    ...
    if model_name in ["ConvJSCCrandomSNR","ResJSCCrandomSNR","SwinJSCCrandomSNR","LICRFJSCCrandomSNR","LAJSCCrandomSNR","FAJSCCrandomSNR"]:
        save_name = f"{random_num}_{task}_{data}_{chan_type}_rcpp{rcpp}_{metric}_{model_name}"
    else:
        save_name = f"{random_num}_{task}_{data}_{chan_type}_SNR{str(SNR).zfill(3)}_rcpp{rcpp}_{metric}_{model_name}"    
...
class Trainer():
    ...
    def train_ITrandomSNR_task(self,cfg: DictConfig, logger, model, trainloader, criterion, optimizer):
        ...
        for images, labels in trainloader:
            ...
            images = images.to(device)
            optimizer.zero_grad()
            SNR_info_list = [1,4,7,10]
            SNR_info = random.sample(SNR_info_list, 1)[0]
