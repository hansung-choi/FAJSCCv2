...
class AttentionPredictor(nn.Module):
    """ Tree structured attention family predictor without shared computation nodes """
    ...
    def forward(self, input_x, mask=None, ratio=0.5, train_mode=False):
        # Channel Attention
        ca = self.ca_head(input_x)

        # Offset
        offsets = self.offset_head(input_x).tanh().mul(8.0)

        # Spatial Attention
        sa = self.sa_head(input_x)

        # Mask prediction
        x_mask = self.mask_branch(input_x)
...
class FAmixerwoAT(nn.Module):
    def __init__(self, dim, window_size=8, bias=True, is_deformable=True, ratio=0.5):
        ...
        self.act = nn.GELU()
        # Predictor
        self.route = AttentionPredictor(dim,window_size,ratio=ratio)
...
class FABlockwoAT(nn.Module):
    def __init__(self, n_feats, window_size=8, ratio=0.5):
        super(FABlockwoAT,self).__init__()
        
        self.n_feats = n_feats
        self.norm1 = LayerNorm(n_feats)
        self.mixer = FAmixerwoAT(n_feats,window_size=window_size,ratio=ratio)
...
class FAGroupwoAT(nn.Module):
    def __init__(self, n_feats, n_block, window_size=8, ratio=0.5):
        super(FAGroupwoAT, self).__init__()
        ...
        self.body = nn.ModuleList([FABlockwoAT(n_feats, window_size=window_size, ratio=ratio) for i in range(n_block)])
