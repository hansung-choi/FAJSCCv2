...
class AttentionTreePGB(nn.Module):
    """ Tree structured attention family predictor with partial gradient blocking for mask branch"""
    ...
    def forward(self, input_x, mask=None, ratio=0.5, train_mode=False):
        ...
        # Mask prediction
        x_mask = self.mask_branch(x_attn.clone().detach()) #PGB mechanism
...
class FAPGBmixer(nn.Module):
    def __init__(self, dim, window_size=8, bias=True, is_deformable=True, ratio=0.5):
        ...
        self.act = nn.GELU()
        # Predictor
        self.route = AttentionTreePGB(dim,window_size,ratio=ratio)
...
class FAPGBBlock(nn.Module):
    def __init__(self, n_feats, window_size=8, ratio=0.5):
        super(FAPGBBlock,self).__init__()
        
        self.n_feats = n_feats
        self.norm1 = LayerNorm(n_feats)
        self.mixer = FAPGBmixer(n_feats,window_size=window_size,ratio=ratio)
...
class FAPGBGroup(nn.Module):
    def __init__(self, n_feats, n_block, window_size=8, ratio=0.5):
        ...
        self.body = nn.ModuleList([FAPGBBlock(n_feats, window_size=window_size, ratio=ratio) for i in range(n_block)])
