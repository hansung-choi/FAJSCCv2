...
class AttentionTreewoLA(nn.Module):
    ...
    def forward(self, input_x, mask=None, ratio=0.5, train_mode=False):
        ...
        if self.training or train_mode:
            ...
            return [idx1, idx2], offsets

#need to implement get_spatial_mask method.
class FAmixerwoLA(nn.Module):
    def __init__(self, dim, window_size=8, bias=True, is_deformable=True, ratio=0.5):
        ...
        self.act = nn.GELU()
        # Predictor
        self.route = AttentionTreewoLA(dim,window_size,ratio=ratio)
    ...
    def forward(self,x,condition_global=None, mask=None, train_mode=False):
        ...
        mask, offsets = self.route(_condition,ratio=self.ratio,train_mode=train_mode) #mask.size() = B X N X 1, N=(h w)
        v = v
        vs = v
        ...
        out = attn_out
        out = self.act(self.conv_sptial(out)) + out
...
class FABlockwoLA(nn.Module):
    def __init__(self, n_feats, window_size=8, ratio=0.5):
        super(FABlockwoLA,self).__init__()
        
        self.n_feats = n_feats
        self.norm1 = LayerNorm(n_feats)
        self.mixer = FAmixerwoLA(n_feats,window_size=window_size,ratio=ratio)
...
class FAGroupwoLA(nn.Module):
    def __init__(self, n_feats, n_block, window_size=8, ratio=0.5):
        super(FAGroupwoLA, self).__init__()
        ...
        self.body = nn.ModuleList([FABlockwoLA(n_feats, window_size=window_size, ratio=ratio) for i in range(n_block)])
